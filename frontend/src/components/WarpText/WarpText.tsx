/*
 * REAL React Bits "WarpText" component.
 * Source: https://github.com/DavidHDev/react-bits/blob/main/src/content/TextAnimations/WarpText/WarpText.jsx
...
const syncUniforms = (program: Program, props: WarpTextRasterProps): void => {
...
export function WarpText({
  ...
  const propsRef = useRef<WarpTextRasterProps>({
    text,
    color,
    fontSize,
    fontWeight,
  ...
  useEffect(() => {
    ...
    const onReducedMotion = (event: MediaQueryListEvent) => {
      ...
      if (program) {
        (program.uniforms.uMotion.value as number) = reduceMotion ? 0 : 1;
      }
      renderOnce();
    };
    ...
    const loop = (now: number) => {
      ...
      (program.uniforms.uPointerActive.value as number) = reduceMotion
        ? pointer.active * 0.35
        : pointer.active;
      (program.uniforms.uTime.value as number) = reduceMotion ? 0 : elapsed;
    ...
    const rasterizeFn = rasterize;
    contextRef.current = { program, rasterize: () => void rasterizeFn() };
    resize();
    raf = requestAnimationFrame(loop);
    ...
    return () => {
      ...
      if (!contextLost && gl) {
        try {
          const tex = (texture as unknown as { texture?: WebGLTexture | null })?.texture;
