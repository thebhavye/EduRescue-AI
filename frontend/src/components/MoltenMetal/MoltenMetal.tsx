/*
 * REAL React Bits "Molten Metal" background component.
 * Source: https://github.com/DavidHDev/react-bits/blob/main/src/content/Backgrounds/MoltenMetal/MoltenMetal.jsx
 * (MIT + Commons Clause — vendored here, converted from JSX to TypeScript without
 * changing the WebGL shader or behaviour; only types were added.)
 * Live reference: https://reactbits.dev/backgrounds/molten-metal
 * Requires the `ogl` package (same dependency the upstream component uses).
 */
...
export function MoltenMetal({
  ...
  useEffect(() => {
    ...
    // Guard only: jsdom / WebGL-less environments have no canvas 2D/WebGL
    // context. Upstream React Bits assumes a real browser; without this the
    // whole app would crash on mount there. With WebGL present the real
    // shader path below runs unchanged.
  ...
  useEffect(() => {
    ...
    (u.uScale.value as number) = scale;
    (u.uDetail.value as number) = detail;
    (u.uGlow.value as number) = glow;
    (u.uCoreSize.value as number) = Math.max(coreSize, 0.001);
    (u.uSwirl.value as number) = swirl;
    (u.uFold.value as number) = fold;
    (u.uBlackPoint.value as number) = blackPoint;
    (u.uBrightness.value as number) = brightness;
    (u.uColorMode.value as number) = colorModeToFloat(colorMode);
    ...
    (u.uLightMode.value as boolean) = lightMode;
